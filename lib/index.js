...
export default function SeriesDisplay(ghostContentApi) {
    ...
    async function displaySeriesInfo(document, options) {
        ...
        for (const insertion of insertions) {
            ...
            // We need to clone the fragment if we have more than one element matching the selector.
            const fragments = [
                fragment, 
                ...times(
                    elements.length - 1, 
            ...
            for (let i = 0; i < elements.length; i++) {
                insertNodeRelativeToElement(fragments[i], elements[i], insertion.position);
            }
        }
    }
