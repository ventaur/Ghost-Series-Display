...
export function SeriesDisplay(ghostContentApi) {
    ...
    async function buildSeriesInfoFragment(document, options) {
        ...
        const { currentPostUrl, hideSinglePostSeries, comingSoonText, seriesTagSlugs } = normailizeBuildOptions(options, () => findSeriesTagSlugs(document));
        ...
        for (const [ seriesTag, posts ] of postsGroupedBySeriesTags) {
            ...
            for (const post of posts) {
                ...
                if (post.url === currentPostUrl) {
                    ...
                    // TODO: Use relative URLs by calling the settings endpoint to get the site URL and remove it from post URLs.
                    // * const rootPath = settings.url;
                    // * link.href = stripSlash(currentUrl.replace(rootPath, ''));
                    link.href = post.url;
