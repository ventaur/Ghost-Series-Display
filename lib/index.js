...
export default function SeriesDisplay(ghostContentApi) {
    ...
    function throwIfDocumentInvalid(document) {
        ...
        if (Document !== undefined && Document.prototype !== Object.getPrototypeOf(document)) {
            throw new TypeError('document must be provided and be an instance of Document.');
        }
    }
     ...
     * @returns {DocumentFragment} A document fragment containing series information.
     */
     ...
     */
    async function displaySeriesInfo(document, seriesTagSlugs, options) {
        throwIfDocumentInvalid(document);
        ...
        const fragment = await buildSeriesInfoFragment(seriesTagSlugs, options);
