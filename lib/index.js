...
export default function SeriesDisplay(ghostContentApi) {
    ...
    async function displaySeriesInfo(document, options) {
        ...
        const fragment = await buildSeriesInfoFragment(document, options);
        ...
        for (const insertion of insertions) {
            ...
            fragments = [...fragments, ...times(fragmentCloneCount, () => fragment.cloneNode(true)) ];
            
            for (const element of elements) {
                // TODO: Build a list of these actions to run later (delayed) because adding a DocumentFragment moves its child nodes, preventing them from being cloned.
                insertNodeRelativeToElement(fragments.pop(), element, insertion.position);
            }
        }
    }
